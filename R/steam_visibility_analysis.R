...
cat("Participants with visibility events:", n_distinct(steam_visibility$pid), "\n")
...
visibility_from_events <- map_dfr(
  ...
  function(participant_id) {
    ...
    if (nrow(participant_info) == 0) return(NULL)
    ...
    total_hours_public <- total_hours_public + hours_public_from_gaming_during_private
...
steam_visibility_summary <- all_gaming_participants |>
  ...
  mutate(
    ...
    # Expected total
    expected_total_hours = as.numeric(difftime(study_end_datetime, enrollment_datetime, units = "hours")),
    ...
    total_hours_public = case_when(
      has_visibility_events ~ pmax(hours_public_from_events, hours_with_gaming_data),
    ...
    hours_public_no_gaming = case_when(
      has_visibility_events ~ pmax(0, hours_public_from_events - hours_with_gaming_data),
      hours_with_gaming_data > 0 ~ expected_total_hours - hours_with_gaming_data,  # Assume public entire period
      TRUE ~ 0
    ),
    ...
    proportion_public_with_gaming = hours_public_with_gaming / expected_total_hours,
  ...
  select(
    pid, enrollment_datetime, study_end_datetime,
    hours_public_with_gaming, hours_public_no_gaming, total_hours_public,
    total_hours_private, hours_unknown, expected_total_hours,
    has_visibility_events, n_visibility_changes,
    proportion_public_with_gaming, proportion_public_no_gaming,
    proportion_hours_public, proportion_hours_private, proportion_unknown
  ) |>
  rename(
...
cat("Participants with visibility events:", sum(steam_visibility_summary$has_visibility_events), "\n")
cat("Participants without visibility events:", sum(!steam_visibility_summary$has_visibility_events), "\n\n")

cat("--- Time-based metrics (hours, all participants) ---\n")
cat("Average hours with gaming data:",
    round(mean(steam_visibility_summary$hours_public_with_gaming, na.rm = TRUE), 1), "\n")
cat("Average hours public without gaming (from visibility events):",
    round(mean(steam_visibility_summary$hours_public_no_gaming, na.rm = TRUE), 1), "\n")
cat("Average hours known private:",
    round(mean(steam_visibility_summary$total_hours_private, na.rm = TRUE), 1), "\n")
cat("Average hours unknown status:",
...
cat("Average hours public without gaming:",
    round(mean(with_events$hours_public_no_gaming, na.rm = TRUE), 1), "\n")
cat("Average hours private:",
    round(mean(with_events$total_hours_private, na.rm = TRUE), 1), "\n")
cat("Average number of visibility changes:",
    round(mean(with_events$n_visibility_changes, na.rm = TRUE), 1), "\n\n")

cat("--- Proportions of study period (84 days = 2016 hours) ---\n")
cat("Average proportion time with gaming:",
    round(mean(steam_visibility_summary$proportion_public_with_gaming, na.rm = TRUE), 3), "\n")
cat("Median proportion time with gaming:",
    round(median(steam_visibility_summary$proportion_public_with_gaming, na.rm = TRUE), 3), "\n")
cat("Average proportion public (no gaming):",
    round(mean(steam_visibility_summary$proportion_public_no_gaming, na.rm = TRUE), 3), "\n")
cat("Average proportion private:",
    round(mean(steam_visibility_summary$proportion_hours_private, na.rm = TRUE), 3), "\n")
cat("Average proportion unknown:",
...
participants_with_survey <- unique(c(participants_with_diary, participants_with_panel))
...
cat("  - With at least one diary:", length(intersect(steam_participants, participants_with_diary)), "\n")
cat("  - With at least one panel:", length(intersect(steam_participants, participants_with_panel)), "\n\n")
...
visibility_timing <- map_dfr(
  ...
  function(participant_id) {
    ...
    events_during <- events |> filter(timestamp >= enroll_dt, timestamp <= end_dt)
...
n_any <- sum(visibility_timing$has_events_before |
...
cat("  - BEFORE study period:", n_before, sprintf("(%.1f%%)", 100 * n_before / n_total), "\n")
cat("  - DURING study period:", n_during, sprintf("(%.1f%%)", 100 * n_during / n_total), "\n")
cat("  - AFTER study period:", n_after, sprintf("(%.1f%%)", 100 * n_after / n_total), "\n")
cat("  - ANY time (before/during/after):", n_any, sprintf("(%.1f%%)", 100 * n_any / n_total), "\n")
cat("  - NO visibility events:", n_none, sprintf("(%.1f%%)", 100 * n_none / n_total), "\n\n")
...
visibility_patterns <- visibility_timing |>
  mutate(
    pattern = case_when(
      ...
      has_events_before & !has_events_during & !has_events_after ~ "Before only",
      !has_events_before & has_events_during & !has_events_after ~ "During only",
      !has_events_before & !has_events_during & has_events_after ~ "After only",
      has_events_before & has_events_during & !has_events_after ~ "Before + During",
      has_events_before & !has_events_during & has_events_after ~ "Before + After",
      !has_events_before & has_events_during & has_events_after ~ "During + After",
      has_events_before & has_events_during & has_events_after ~ "All three periods",
      TRUE ~ "Other"
    )
  ) |>
  count(pattern) |>
...
cat("  - Before study period:", round(mean(visibility_timing$n_events_before), 1), "\n")
cat("  - During study period:", round(mean(visibility_timing$n_events_during), 1), "\n")
cat("  - After study period:", round(mean(visibility_timing$n_events_after), 1), "\n\n")

# For participants with events during study period only
with_during_events <- visibility_timing |> filter(has_events_during)
cat("For participants with visibility events DURING study period (n=", nrow(with_during_events), "):\n", sep="")
cat("  - Average events during:", round(mean(with_during_events$n_events_during), 1), "\n")
cat("  - Median events during:", round(median(with_during_events$n_events_during), 1), "\n")
cat("  - Range:", min(with_during_events$n_events_during), "to", max(with_during_events$n_events_during), "\n\n")
...
cat("We only analyze visibility events that occurred DURING the 84-day study period.\n\n")
...
hidden_status <- map_dfr(
  ...
  function(participant_id) {
    ...
    status <- case_when(
      ...
      has_private_events & !has_gaming_data ~ "Confirmed hidden (private events, no gaming data)",

      # Public with private periods: has gaming data (proves public) AND has private events
      has_gaming_data & has_private_events ~ "Public with private periods (gaming data + private events)",

      # Confirmed public only: has gaming data OR only public events, no private events
      (has_gaming_data | has_public_events) & !has_private_events ~ "Confirmed public only (gaming data or public events only)",

      # Unknown: no visibility events and no gaming data
      !has_visibility_events & !has_gaming_data ~ "Unknown (no visibility tracking, no gaming data)",
...
cat("  - With gaming data during study:", n_with_gaming,
    sprintf("(%.1f%%)", 100 * n_with_gaming / n_total), "\n")
cat("  - No gaming data during study:", n_no_gaming,
    sprintf("(%.1f%%)", 100 * n_no_gaming / n_total), "\n")
cat("  - With visibility events during study:", n_with_visibility,
    sprintf("(%.1f%%)", 100 * n_with_visibility / n_total), "\n")
cat("  - No visibility events during study:", n_no_visibility,
...
cat("Key insights:\n")
cat("  - Confirmed hidden (private events, no gaming):", nrow(confirmed_hidden), "\n")
...
cat("    → These participants had gaming data (proves public) but also had private periods\n")
cat("  - Confirmed public only:", nrow(confirmed_public), "\n")
cat("    → These participants had gaming data and/or only public visibility events\n\n")

# Gaming visibility analysis ----------------------------------------------

cat("--- Gaming Visibility Analysis ---\n\n")
cat("This section estimates how much gaming activity was visible vs invisible to researchers.\n\n")
...
cat("Survey-active Steam participants in visibility summary:", nrow(survey_active_visibility), "\n\n")
...
cat("  Total hours in study:", round(mean(survey_active_visibility$expected_total_hours), 1), "\n\n")

cat("  Hours WITH gaming data:", round(mean(survey_active_visibility$hours_with_gaming), 1),
    sprintf("(%.1f%%)", 100 * mean(survey_active_visibility$hours_with_gaming / survey_active_visibility$expected_total_hours)), "\n")
cat("    → Profile was definitely public during these hours\n\n")

cat("  Hours WITHOUT gaming data:", round(mean(survey_active_visibility$hours_no_gaming), 1),
    sprintf("(%.1f%%)", 100 * mean(survey_active_visibility$hours_no_gaming / survey_active_visibility$expected_total_hours)), "\n")
cat("    → Could be: (a) private/hidden, (b) public but not playing, or (c) unknown\n\n")

cat("    Breakdown of hours without gaming:\n")
cat("      - Confirmed private (from visibility events):", round(mean(survey_active_visibility$hours_confirmed_private), 1), "\n")
cat("      - Unknown status (no visibility tracking):", round(mean(survey_active_visibility$hours_unknown_status), 1), "\n\n")
...
cat("Totals across all", nrow(survey_active_visibility), "survey-active participants:\n")
cat("  Total hours in study period:", format(total_expected, big.mark=","), "\n")
cat("  Hours with gaming data:", format(total_with_gaming, big.mark=","),
    sprintf("(%.1f%%)", 100 * total_with_gaming / total_expected), "\n")
cat("  Hours without gaming data:", format(total_no_gaming, big.mark=","),
    sprintf("(%.1f%%)", 100 * total_no_gaming / total_expected), "\n")
cat("    - Confirmed private:", format(total_confirmed_private, big.mark=","),
    sprintf("(%.1f%%)", 100 * total_confirmed_private / total_expected), "\n")
cat("    - Unknown status:", format(total_unknown, big.mark=","),
...
without_events_during <- survey_active_visibility |> filter(!has_visibility_events)

cat("Comparison by visibility event availability:\n\n")

cat("Participants WITH visibility events during study (n=", nrow(with_events_during), "):\n", sep="")
cat("  Hours with gaming:", round(mean(with_events_during$hours_with_gaming), 1), "\n")
cat("  Hours confirmed private:", round(mean(with_events_during$hours_confirmed_private), 1), "\n")
cat("  Hours unknown:", round(mean(with_events_during$hours_unknown_status), 1), "\n\n")

cat("Participants WITHOUT visibility events during study (n=", nrow(without_events_during), "):\n", sep="")
cat("  Hours with gaming:", round(mean(without_events_during$hours_with_gaming), 1), "\n")
cat("  Hours confirmed private:", round(mean(without_events_during$hours_confirmed_private), 1), "\n")
cat("  Hours unknown:", round(mean(without_events_during$hours_unknown_status), 1), "\n")
cat("  Note: Most hours are 'unknown' - we only know they were public when we have gaming data\n\n")
...
unknown_pids <- hidden_status$pid[hidden_status$status == "Unknown (no visibility tracking, no gaming data)"]
...
if (nrow(public_with_private_visibility) > 0) {
  cat("Public with private periods (n=", nrow(public_with_private), "):\n", sep="")
  cat("  Total hours in study period:",
      format(sum(public_with_private_visibility$expected_total_hours), big.mark=","), "\n")
  cat("  Hours with gaming data:",
      format(sum(public_with_private_visibility$hours_with_gaming), big.mark=","), "\n")
  cat("  Hours confirmed private (hidden):",
      format(sum(public_with_private_visibility$hours_confirmed_private), big.mark=","), "\n")
  cat("  Hours unknown:",
      format(sum(public_with_private_visibility$hours_unknown_status), big.mark=","), "\n")
  cat("  → Total hours hidden or unknown:",
      format(sum(public_with_private_visibility$hours_confirmed_private +
                 public_with_private_visibility$hours_unknown_status), big.mark=","),
      sprintf("(%.1f%% of their study time)\n\n",
              100 * sum(public_with_private_visibility$hours_confirmed_private +
...
if (n_unknown > 0) {
  ...
  cat("Unknown (no visibility tracking, no gaming data) (n=", n_unknown, "):\n", sep="")
  cat("  Total hours in study period:",
...
total_hours_combined <- sum(public_with_private_visibility$expected_total_hours) +
                        (n_unknown * 2016)
total_hidden_unknown_combined <- sum(public_with_private_visibility$hours_confirmed_private +
                                      public_with_private_visibility$hours_unknown_status) +
                                  (n_unknown * 2016)
...
p1 <- ggplot(steam_visibility_summary, aes(x = proportion_public_with_gaming)) +
  ...
  labs(
    ...
    subtitle = sprintf("Proportion of 84-day study period with gaming data (N=%d participants)",
...
summary_by_event_status <- steam_visibility_summary |>
  mutate(event_group = ifelse(has_visibility_events,
  ...
  pivot_longer(cols = c(Gaming, `Public (no gaming)`, Private, Unknown),
               names_to = "Status", values_to = "Proportion") |>
  mutate(
    Status = factor(Status, levels = c("Gaming", "Public (no gaming)", "Private", "Unknown")),
    label = sprintf("%s\n(n=%d)", event_group, n)
  )

p2 <- ggplot(summary_by_event_status, aes(x = label, y = Proportion, fill = Status)) +
...
cat("The calculate_steam_visibility_coverage() function calculates visibility for individual participants.\n\n")
...
if (length(example_pids) > 0) {
  ...
  for (i in seq_along(example_pids)) {
    ...
    cat("  Proportion visible:", sprintf("%.1f%%", 100 * result$proportion_visible), "\n")
