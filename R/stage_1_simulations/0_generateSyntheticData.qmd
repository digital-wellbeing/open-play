...
::: {.callout-note}
This file is deprecated. It previously synthetically sampled from real data, with PII and thus internal only, and saved it's outputs to `data/data-synthetic-raw`. These files are fixed and this no longer needs to be run, but is saved for posterity to have a record of the data generating process. 
:::
