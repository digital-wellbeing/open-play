...
steam_clean <- steam |>
  ...
  ungroup() |> 
  select(pid, timestamp, date, hour, title_id, steam_id, playtime_forever, playtime_2weeks, game_ref)
...
intake_clean <- intake |>
  ...
  mutate(
    linked_platforms = {
      ...
      v <- v[v != ""]
      if (pid %in% ios$pid)     v <- union(v, "iOS")
      if (pid %in% android$pid) v <- union(v, "Android")
