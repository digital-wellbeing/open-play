...
intake_clean <- intake |>
  ...
  ungroup() |>
  select(pid:edu_level, marital_status, political_party, employment, everything())
...
daily_clean <- daily_clean |>
  select(-baseline_day, -ideal_baseline_day, -n_valid, -original_wave) |>
  select(pid, wave, date, everything())
...
biweekly_clean <- biweekly |>
  ...
  mutate(baseline_day = as.Date(date) - if_else(hour(date) < 4, 1L, 0L)) %>%
  {
    ...
    base |>
      ...
      ungroup() |>
      select(
        -ideal_baseline_day, -baseline_day,
        -original_wave, -derived_wave,
        -n_valid, -delta_days_final
      ) |>
      select(pid, wave, date, everything())
  }
...
timeuse_flags <- timeuse |>
  ...
  mutate(
    quality_flag = case_when(
      ...
      n_activities < 5 ~ "Low quality: fewer than 5 activities logged",
      TRUE ~ "Good quality"
    )
  ) |>
