...
steam_quality_metrics <- steam_expanded_for_metrics |>
  ...
  summarise(
    ...
    hours_exceeding_60 = n_distinct(pid[hour_total > 60], hour_start_utc[hour_total > 60]),
...
```{r}
#| label: preprocess-timeuse
# Assign quality flags to daily time use diaries
# 0: Good quality
# 1: 90+ minutes missing
# 2: No sleep
# 3: No personal care  
# 4: No sleep and no personal care

# Calculate duration and flags
timeuse_flags <- timeuse |>
  mutate(
    start_dt = ymd_hms(start_time, tz = "UTC"),
    end_dt = ymd_hms(end_time, tz = "UTC"),
  ...
  summarise(
    ...
    has_personal_care = any(str_to_lower(activity) == "personal care", na.rm = TRUE),
    .groups = "drop"
  ) |>
  mutate(
    quality_flag = case_when(
      !has_sleep & !has_personal_care ~ 4L,
      !has_sleep ~ 2L,
      !has_personal_care ~ 3L,
      total_minutes < 1350 ~ 1L,  # 1440 - 90 = 1350
      TRUE ~ 0L
    )
  ) |>
