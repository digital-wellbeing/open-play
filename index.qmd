...
tibble(
  ...
  `Account Linking Process` = c(
    ...
    "Participants sign up for Gameplay.Science (https://gameplay.science), an open-source platform for tracking Steam gameplay. Participants consent to have their gameplay data monitored for the duration of the study. Their Steam ID is authenticated using the official Steam authentication API (OpenID).",
...
biweekly_wellbeing <- biweekly |>
  ...
  filter(!is.na(wemwbs) & wemwbs >= 14) |>  # Remove invalid/missing wellbeing scores
  mutate(
...
# Wellbeing axis will be fixed at 14-70 across all biweeklies
...
for (i in seq_along(pct_levels)) {
  ...
  # Get biweekly-specific max playtime
  biweekly_max <- biweekly_max_playtime |> filter(percentile == pct_level) |> pull(max_playtime)

  # Calculate biweekly-specific coefficient for dual axis
  # Wellbeing is fixed at 14-70 (range of 56), gaming time y-axis is free per biweekly
  wb_range <- 56  # Fixed wellbeing range (70 - 14)

  # The coefficient determines how wellbeing values map to the playtime y-axis
  # We set it based on biweekly_max, but need to ensure y-axis limit shows full wellbeing range
  coeff <- biweekly_max / wb_range

  # Calculate the y-axis maximum to ensure wellbeing 14-70 is fully visible
  # We need the y-axis to extend to at least (70-14)*coeff in playtime units
  # Add more buffer to ensure the 70 tick is displayed by ggplot2
  # Using a larger multiplier to give more headroom
  y_for_wb70 <- (70 - 14) * coeff  # Playtime value corresponding to wellbeing=70
  y_axis_max <- y_for_wb70 + (y_for_wb70 * 0.15)  # Add 15% headroom above wellbeing=70
...
```{r}
#| label: fig-time-use
#| fig-cap: "Illustrative single-day overview of one study day, consisting of a participant's time use diary (top panel), gaming as recorded through telemetry (middle panel), and survey completion (bottom panel)."
#| fig-asp: 0.4
#| out.width: 95%
