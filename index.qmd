...
prop_failed_daily_attn <- number(
  sum(daily$bpnsfs_failed_att_check) / nrow(daily) * 100,
  accuracy = .1
)
prop_failed_biweekly_attn <- number(
  sum(biweekly$bangs_failed_att_check) / nrow(biweekly) * 100,
  accuracy = .1
)
...
overview <- bind_rows(
  ...
  rename(`N survey participants` = Participants)
...
sankey_data <- intake |>
  mutate(
    ...
    Account_Validation_1 = ifelse(
      pid %in% c(xbox$pid, nintendo$pid, steam$pid),
      "Any trace data",
      "No trace data"
    ),
    Account_Validation_2 = ifelse(
      qualified,
      "Recent trace data",
      "No recent trace data"
    ),
  ...
  make_long(
    Screening,
    Linking,
    Account_Validation_1,
    Account_Validation_2,
    Surveys
  ) |>
  left_join(node_counts, by = "node") |>
  # map internal names to pretty labels only for display
  mutate(
    node_pretty = recode(
      node,
      "Account_Validation_1" = "Account\nValidation 1",
      "Account_Validation_2" = "Account\nValidation 2",
      .default = node
    ),
    next_node_pretty = recode(
      next_node,
      "Account_Validation_1" = "Account\nValidation 1",
      "Account_Validation_2" = "Account\nValidation 2",
      .default = next_node
    ),
    node = factor(node_pretty, levels = rev(lvl)),
    next_node = factor(next_node_pretty, levels = rev(lvl)),
    node_label = sprintf("%s\n%s", as.character(node), n)
  )
...
sessions_df <- sessions_all |>
  ...
  transmute(
    ...
    start = as.POSIXct(session_start, tz = "UTC") +
      minutes(offset_secs(tz_str) / 60),
    end = as.POSIXct(session_end, tz = "UTC") +
      minutes(offset_secs(tz_str) / 60)
  ) |>
  clip_to_window()
...
activity_colors <- c(
  "Sleep" = "#111111",
  "Personal Care" = "#666666",
  "Video Gaming" = "#107C10",
  "Offline Leisure & Social Life" = "#999999",
  "Other Digital Media Use" = "#444444"
)
platform_colors <- c(
  "Xbox" = "#107C10",
  "Nintendo" = "#E60012",
...
While not exhaustive, the data we share in clean implements a variety of data quality checks. Specifically, in each daily and biweekly survey, one item from the BANGS (daily) and BPNSFS (biweekly) was duplicated to assess response consistency [@MeadeCraig2012identifying]; participants whose responses to the two identical items differed by more than one scale point were flagged for potential careless responding (daily surveys: `r prop_failed_daily_attn`% flagged, biweekly: `r prop_failed_biweekly_attn`%).
...
active_daily <- intake |>
  ...
  inner_join(
    daily |> count(pid, name = "n_daily") |> filter(n_daily >= 1),
    by = "pid"
  )
...
active_bw <- intake |>
  transmute(pid, country) |>
  inner_join(
    biweekly |> count(pid, name = "n_bw") |> filter(n_bw >= 1),
    by = "pid"
  )
...
expected_notes <- list(
  "a" = list(
    i = 1,
    j = 5,
    text = "Difference reflects users who were eligible but did not complete any post-intake surveys, and thus only have telemetry data"
  ),
  "b" = list(
    i = footnote_mapping$row_num[footnote_mapping$footnote == "a"],
    j = 6,
    text = "Max. possible survey obs.: N participants × N surveys (30 for Daily, 6 for Biweekly)"
  ),
  "c" = list(
    i = footnote_mapping$row_num[footnote_mapping$footnote == "b"],
    j = 6,
    text = "Max. possible Nintendo/Xbox obs.: N participants × ≥1 session"
  ),
  "d" = list(
    i = footnote_mapping$row_num[footnote_mapping$footnote == "c"],
    j = 6,
    text = "Max. possible Steam obs.: N Participants × 84 days of collection × 24 hours"
  ),
  "e" = list(
    i = footnote_mapping$row_num[footnote_mapping$footnote == "d"],
    j = 6,
    text = "Max. possible iOS/Android obs.: N Participants × 84 days"
  ),
  "f" = list(
    i = footnote_mapping$row_num[footnote_mapping$footnote == "e"],
    j = 6,
    text = "Max. possible Cognitive test obs.: N participants × 3 test administrations"
  )
)

tt(
  table_data |> select(-Retention),
