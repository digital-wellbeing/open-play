...
propFailedDiaryAttnCheck <- number(sum(diary$bpnsfs_failed_att_check)/nrow(diary)*100, accuracy = .1)
propFailedPanelAttnCheck <- number(sum(panel$bangs_failed_att_check)/nrow(panel)*100, accuracy = .1)
...
# Identify participants who completed between 1 and 6 biweekly panel responses
# Exclude participants with 0 or more than 6 panel responses
panel_participants <- panel |>
  ...
  filter(n_panels >= 1 & n_panels <= 6) |>  # Only 1-6 panel responses
  mutate(pid = as.character(pid))

# Join study start dates and calculate study day
# Only include participants who completed between 1-6 biweekly panels
daily_with_study_day <- daily_all |>
  inner_join(panel_participants |> select(pid, study_start), by = "pid") |>
  mutate(
...
# Only include participants who completed at least one biweekly panel
...
# Calculate panel-specific max playtime for free gaming time y-axis
# Wellbeing axis will be fixed at 14-70 across all panels
panel_max_playtime <- plot_data_complete |>
...
for (i in seq_along(pct_levels)) {
  ...
  # Get panel-specific max playtime
  panel_max <- panel_max_playtime |> filter(percentile == pct_level) |> pull(max_playtime)

  # Calculate panel-specific coefficient for dual axis
  # Wellbeing is fixed at 14-70 (range of 56), gaming time y-axis is free per panel
  wb_range <- 56  # Fixed wellbeing range (70 - 14)

  # The coefficient determines how wellbeing values map to the playtime y-axis
  # We set it based on panel_max, but need to ensure y-axis limit shows full wellbeing range
  coeff <- panel_max / wb_range
...
1.  In each daily and biweekly survey, one item from the BANGS (daily) and BPNSFS (biweekly) was duplicated to assess response consistency [@MeadeCraig2012identifying]; participants whose responses to the two identical items differed by more than one scale point were flagged for potential careless responding (daily surveys: `r propFailedDiaryAttnCheck`% flagged, biweekly: `r propFailedPanelAttnCheck`).
...
table_data <- missingness_table |>
  mutate(
    ...
    `Median Missing` = ifelse(
      ...
      "—",
      sprintf("%.0f", floor(median_missing_per_participant))
    ),
...
if (knitr::is_html_output()) {
  ...
  tbl_html <- tt(table_data) |>
    ...
    style_tt(j = c(8, 9), align = "c") |>
    style_tt(fontsize = 0.8)

  # Apply retention plots to all rows EXCEPT UK Xbox
