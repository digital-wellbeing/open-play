...
## ---- telemetry eligibility (denominators) ----------------------------------
## Eligible = (>1 Daily OR >1 Biweekly) AND appears in that telemetry table
elig_union <- bind_rows(active_daily, active_bw) |>
  distinct(pid, country)
...
rows <- map_dfr(regions, \(rg) {
  ...
  r_daily <- {
    obs <- daily |>
      filter(pid %in% pids_daily) |>
      count(pid, name = "obs")
    base <- tibble(pid = pids_daily) |>
      left_join(obs, by = "pid") |>
      mutate(obs = coalesce(obs, 0L), miss = E_DAILY - obs)
    fmt_row(rg, "Survey", "Daily Diary", length(pids_daily),
            length(pids_daily) * E_DAILY, base$miss)
  }

  r_bw <- {
    obs <- biweekly |>
      filter(pid %in% pids_bw) |>
      count(pid, name = "obs")
    base <- tibble(pid = pids_bw) |>
      left_join(obs, by = "pid") |>
      mutate(obs = coalesce(obs, 0L), miss = E_BIWEEKLY - obs)
    fmt_row(rg, "Survey", "Biweekly", length(pids_bw),
            length(pids_bw) * E_BIWEEKLY, base$miss)
  }

  ## telemetry eligible pids for this region
  elig_rg <- eligible_tel |> filter(country == rg)

  ## Xbox/Nintendo: binary completeness (expected = eligible count; observed = has any data)
  bin_row <- \(meas, df_tel) {
    p <- elig_rg |> filter(measure == meas) |> pull(pid)
    ## since eligibility already requires appearing in the table, observed == eligible
    miss_vec <- rep.int(0L, length(p))
    fmt_row(rg, "Telemetry", meas, length(p), length(p), miss_vec)
  }

  r_xbox <- bin_row("Xbox", xbox)
  r_ntd <- bin_row("Nintendo", nintendo)

  ## iOS/Android: day counts vs 84
  days_row <- \(meas, df_days) {
    ...
    fmt_row(rg, "Telemetry", meas, length(p), length(p) * D_MOBILE, base$miss)
  }

  r_ios <- days_row("iOS", ios)
  r_and <- days_row("Android", android)

  ## Steam: hours_visible vs 2016
  r_steam <- {
    p <- elig_rg |> filter(measure == "Steam") |> pull(pid)
    hrs <- steam_windows |>
      right_join(tibble(pid = p), by = "pid") |>
      mutate(hours_visible = coalesce(hours_visible, 0)) |>
      pull(hours_visible)
    miss <- pmax(0, H_STEAM - hrs)
    fmt_row(rg, "Telemetry", "Steam", length(p), length(p) * H_STEAM, miss)
  }

  ## Simon: tie to biweekly cohort (common in your prior setup)
  r_simon <- {
    obs <- simon |>
      filter(pid %in% pids_bw) |>
      count(pid, name = "obs")
    ...
    fmt_row(rg, "Task", "Simon Task", length(pids_bw),
            length(pids_bw) * E_SIMON, base$miss)
  }
...
table_data <- rows |>
  ...
  mutate(
    ...
    N = n_fmt(n_participants),
    Expected = n_fmt(total_expected),
    ...
    `% Missing` = sprintf("%.1f%%", 100-pct_complete),
    Retention = ""
  ) |>
  ...
  select(
    ...
    N, Expected, Observed, `Median (Max) Missing`, `% Missing`, Retention,
    show_type, show_region
  ) |>
  mutate(
    Region = if_else(show_region, Region, ""),
    `Data Type` = if_else(show_type, `Data Type`, ""),
    `% Missing` = if_else(`% Missing` == "NaN%", "-", `% Missing`),
  ) |>
...
expected_notes <- list(
  "a" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "a"], j = 5,
             text = "Surveys: Number of participants × expected responses (30 for Daily Diary, 6 for Biweekly Panel)"),
  "b" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "b"], j = 5,
             text = "Telemetry (Xbox/Nintendo): Eligible = >1 Daily or >1 Biweekly and present in the platform feed"),
  "c" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "c"], j = 5,
             text = "Telemetry (Steam): Eligible × 2,016 hours (84 days × 24 hours)"),
  "d" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "d"], j = 5,
             text = "Telemetry (iOS/Android): Eligible × 84 days"),
  "e" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "e"], j = 5,
