...
To prepare daily and biweekly survey data, we removed duplicate entries from within the same survey wave and those that had errors in the coding of waves (e.g., administration of survey wave 31 due to technical issues), in total comprising `r prop_duplicate_daily`% of daily surveys, `r prop_duplicate_biweekly`% of biweekly surveys. We further removed failed `r prop_failed_daily_attn`% of daily surveys and `r prop_failed_biweekly_attn`% of biweekly surveys that failed an attention check (see Data Quality below), resulting in a final dataset of `r number(nrow(daily), big.mark = ",")` daily surveys and `r number(nrow(biweekly), big.mark = ",")` biweekly surveys.
