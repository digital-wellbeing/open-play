...
steam_visibility <- read_csv("data/raw/telemetry_steam_account_linking_raw.csv.gz")
...
diary_df <- time_use |>
  ...
  mutate(activity = fct_relevel(activity,
                                "Video Gaming", "Sleep", "Other Digital Media Use", "Personal Care","Offline Leisure & Social Life",
                                ))
...
ggplot() +
  ...
  geom_segment(data = sessions_df,
               ...
               linewidth = 30, lineend = "butt", show.legend = FALSE) +
  geom_segment(data = survey_df, aes(x = ts, xend = ts, y = 0.6, yend = 3.4),
               linetype = "dashed", linewidth = 1.5, alpha = 0.55) +
  geom_point(data = survey_df, aes(x = ts, y = 1),
             shape = 21, size = 10, stroke = 0.6, fill = "#FF6B35") +
...
steam_windows <- steam_visibility |>
  group_by(pid) |>
  summarise(
    link_start   = if (any(event == "accountLinked"))   min(timestamp[event == "accountLinked"])   else as.POSIXct(NA),
    link_end     = if (any(event == "accountUnlinked")) max(timestamp[event == "accountUnlinked"]) else as.POSIXct(NA),
    public_start = if (any(event == "steamDataPublic")) min(timestamp[event == "steamDataPublic"]) else as.POSIXct(NA),
    private_end  = if (any(event == "steamDataPrivate")) max(timestamp[event == "steamDataPrivate"]) else as.POSIXct(NA),
    .groups = "drop"
  ) |>
  left_join(
    intake |>
      transmute(
        ...
        enroll = coalesce(
          ...
          as.POSIXct(enrollment_datetime, format = "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")
        ),
  ...
  mutate(
    ...
    hours_link = if_else(!is.na(link_start) & !is.na(link_end) & link_end > link_start,
                         as.numeric(difftime(link_end, link_start, "hours")), NA_real_),
    hours_pp   = if_else(!is.na(public_start) & !is.na(private_end) & private_end > public_start,
                         as.numeric(difftime(private_end, public_start, "hours")), NA_real_),
    hours_fallback = if_else(!is.na(window_start) & !is.na(assumed_end) & assumed_end > window_start,
                             as.numeric(difftime(assumed_end, window_start, "hours")), NA_real_),
    hours_visible = coalesce(hours_link, hours_pp, hours_fallback)
  ) |>
  select(pid, hours_visible)

## ---- per-region participants -----------------------------------------------
active_daily <- intake |>
  transmute(pid, country) |>
  inner_join(daily |> count(pid, name = "n_daily") |> filter(n_daily > 1), by = "pid")

active_bw <- intake |>
  transmute(pid, country) |>
  inner_join(biweekly |> count(pid, name = "n_bw") |> filter(n_bw > 1), by = "pid")

## UK Daily anomalies: drop them here
active_daily <- active_daily |> filter(country != "UK")

## ---- telemetry sets & totals -----------------------------------------------
tele_sets <- list(
...
## N total per region/platform (all telemetry pids joined to intake for region)
tele_total <- imap(tele_sets, \(tel_df, nm) {
  tel_df |>
    inner_join(intake |> select(pid, country), by = "pid") |>
    mutate(measure = nm)
}) |> list_rbind()

## Eligible = (>1 Daily OR >1 Biweekly) AND appears in that telemetry table
elig_union <- bind_rows(active_daily, active_bw) |> distinct(pid, country)
eligible_tel <- imap(tele_sets, \(tel_df, nm) {
  elig_union |>
    inner_join(tel_df, by = "pid") |>
    mutate(measure = nm)
}) |> list_rbind()

## ---- build rows per region --------------------------------------------------
rows <- map_dfr(regions, \(rg) {
  ## survey sets (note UK daily = 0 by construction)
  pids_daily <- active_daily |> filter(country == rg) |> pull(pid)
  pids_bw    <- active_bw    |> filter(country == rg) |> pull(pid)
  ...
  r_daily <- {
    ...
    n_total <- daily |>
      inner_join(intake |> filter(country == rg), by = "pid") |>
      distinct(pid) |>
      nrow()
    fmt_row(rg, "Survey", "Daily Diary",
            n_total = n_total,
            n_eligible = length(pids_daily),
            max_possible = length(pids_daily) * E_DAILY,
            miss_vec = base$miss)
  }
  ...
  r_bw <- {
    ...
    n_total <- biweekly |>
      ...
      distinct(pid) |>
      nrow()
    fmt_row(rg, "Survey", "Biweekly",
            n_total = n_total,
            n_eligible = length(pids_bw),
            max_possible = length(pids_bw) * E_BIWEEKLY,
            miss_vec = base$miss)
  }

  ## telemetry eligible & totals for this region
  elig_rg <- eligible_tel |> filter(country == rg)
  total_rg <- tele_total |> filter(country == rg)

  ## Xbox/Nintendo: binary (max_possible = N eligible)
  bin_row <- \(meas) {
    p_eligible <- elig_rg |> filter(measure == meas) |> pull(pid)
    n_total    <- total_rg |> filter(measure == meas) |> distinct(pid) |> nrow()
    miss_vec   <- rep.int(0L, length(p_eligible))  # eligible implies has table presence
    fmt_row(rg, "Telemetry", meas,
            n_total = n_total,
            n_eligible = length(p_eligible),
            max_possible = length(p_eligible),   # binary “maximum possible”
            miss_vec = miss_vec)
  }

  r_xbox <- bin_row("Xbox")
  r_ntd  <- bin_row("Nintendo")

  ## iOS/Android: days vs 84
  days_row <- \(meas, df_days) {
    p_eligible <- elig_rg |> filter(measure == meas) |> pull(pid)
    n_total    <- total_rg |> filter(measure == meas) |> distinct(pid) |> nrow()
    obs <- df_days |>
      filter(pid %in% p_eligible) |>
      count(pid, name = "obs")
    base <- tibble(pid = p_eligible) |>
      left_join(obs, by = "pid") |>
      mutate(obs = coalesce(obs, 0L), miss = pmax(0L, D_MOBILE - obs))
    fmt_row(rg, "Telemetry", meas,
            n_total = n_total,
            n_eligible = length(p_eligible),
            max_possible = length(p_eligible) * D_MOBILE,
            miss_vec = base$miss)
  }

  r_ios <- days_row("iOS", ios)
  r_and <- days_row("Android", android)

  ## Steam: hours vs 2016
  r_steam <- {
    p_eligible <- elig_rg |> filter(measure == "Steam") |> pull(pid)
    n_total    <- total_rg |> filter(measure == "Steam") |> distinct(pid) |> nrow()
    hrs <- steam_windows |>
      right_join(tibble(pid = p_eligible), by = "pid") |>
      mutate(hours_visible = coalesce(hours_visible, 0)) |>
      pull(hours_visible)
    miss <- pmax(0, H_STEAM - hrs)
    fmt_row(rg, "Telemetry", "Steam",
            n_total = n_total,
            n_eligible = length(p_eligible),
            max_possible = length(p_eligible) * H_STEAM,
            miss_vec = miss)
  }

  ## Simon: tied to biweekly cohort
  r_simon <- {
    ...
    n_total <- simon |>
      ...
      distinct(pid) |>
      nrow()
    fmt_row(rg, "Task", "Simon Task",
            n_total = n_total,
            n_eligible = length(pids_bw),
            max_possible = length(pids_bw) * E_SIMON,
            miss_vec = base$miss)
  }

  bind_rows(r_daily, r_bw, r_xbox, r_ntd, r_steam, r_ios, r_and, r_simon)
})

## ---- present in identical table format -------------------------------------
table_data <- rows |>
  filter(!(region == "UK" & measure %in% c("Daily Diary", "Xbox"))) |>
  mutate(
    pct_missing_num = if_else(maximum_possible > 0,
                              100 * (total_missing / maximum_possible),
                              0),
    `N total` = n_fmt(n_total),
    `N with survey data` = n_fmt(n_participants),
    `Max. Possible` = n_fmt(maximum_possible),
    Observed = n_fmt(total_observed),
  ...
  mutate(show_type = row_number() == 1L, show_region = row_number() == 1L) |>
  ungroup() |>
  group_by(region) |>
  mutate(show_region = row_number() == 1L) |>
  ungroup() |>
  select(
    Region = region, `Data Type` = data_type, Measure = measure,
    `N total`, `N with survey data`, `Max. Possible`, Observed,
    `Median (Max) Missing`, `% Missing`, Retention,
    show_type, show_region
  ) |>
  mutate(
    Region = if_else(show_region, Region, ""),
    `Data Type` = if_else(show_type, `Data Type`, "")
  ) |>
  select(-show_type, -show_region)

us_start <- 1L
uk_start <- which(rows$region == "UK")[1L]
...
expected_notes <- list(
  "a" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "a"], j = 6,
             text = "Surveys: Number of participants × expected responses (30 for Daily Diary, 6 for Biweekly Panel)"),
  "b" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "b"], j = 6,
             text = "Telemetry (Xbox/Nintendo): Eligible = >1 Daily or >1 Biweekly and present in the platform feed"),
  "c" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "c"], j = 6,
             text = "Telemetry (Steam): Eligible × 2,016 hours (84 days × 24 hours)"),
  "d" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "d"], j = 6,
             text = "Telemetry (iOS/Android): Eligible × 84 days"),
  "e" = list(i = footnote_mapping$row_num[footnote_mapping$footnote == "e"], j = 6,
             text = "Cognitive Task: Number of participants × 3 test administrations")
)

tt(table_data |> select(-Retention), notes = expected_notes, width = 1, escape = TRUE) |>
  style_tt(i = 0, bold = TRUE) |>
  style_tt(i = c(us_start, uk_start), j = 1, bold = TRUE, line = "t", line_width = 0.15) |>
  style_tt(i = data_type_starts, j = 2, bold = TRUE, line = "t", line_width = 0.1) |>
  style_tt(j = 9, align = "c") |>
  style_tt(fontsize = 0.8)
