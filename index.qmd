...
```{r}
#| label: load-data
#| cache.extra: !expr file.mtime("data/raw-minimal-pseudo/survey_intake_raw.csv.gz")

intake <- read_csv(here("data/raw-minimal-pseudo/survey_intake_raw.csv.gz"))
diary <- read_csv(here("data/raw-minimal-pseudo/survey_diary_raw.csv.gz"))
panel <- read_csv(here("data/raw-minimal-pseudo/survey_panel_raw.csv.gz"))

nintendo <- read_csv(here("data/raw-minimal-pseudo/telemetry_nintendo_raw.csv.gz"))
steam <- read_csv(here("data/raw-minimal-pseudo/telemetry_steam_raw.csv.gz"))
xbox <- read_csv(here("data/raw-minimal-pseudo/telemetry_xbox_raw.csv.gz"))

cog <- read_csv(here("data/raw-minimal-pseudo/cognitive_tasks_raw.csv.gz"))
...
third_party_prop <- paste0(print_num((1-median(intake$nintendo_first_party_prop, na.rm = T))*100, 1), "%")
...
# Move aggregate data columns to own table
dat_cog_total <- dat_cog |>
  filter(practice == 0 & pid %in% intake$pid) |>
  summarise(
    score = mean(score_final),
    meanrt = mean(meanrt_final),
    n = n(),
    .by = c(pid, wave, task)
  ) |>
  pivot_wider(
    names_from = task,
    values_from = c(score, meanrt, n)
  )
```
...
sessions_all <- bind_rows(
  xbox_clean |> 
    transmute(pid = as.character(pid), platform = "Xbox",
              start_utc = session_start, end_utc = session_end),
  nintendo_clean |> 
    transmute(pid = as.character(pid), platform = "Nintendo",
              start_utc = session_start, end_utc = session_end)
) |> 
  ...
  filter(!is.na(start_utc), !is.na(end_utc), end_utc > start_utc, duration_min >= 1)
...
hourly_from_sessions <- sessions_all |> 
  ...
  mutate(
    ...
    minutes = pmax(
      0,
      as.numeric(difftime(pmin(end_local, hour_start_local + hours(1)),
...
total_hours <- function(df) {
  mm <- if ("duration" %in% names(df)) df$duration
        else if ("minutes" %in% names(df)) df$minutes
        else NULL
...
overview <- bind_rows(
  ...
  summ_basic(diary,          pid, "Daily surveys",       avg_days = nrow(diary) / n_distinct(diary$pid)),
  summ_basic(panel,          pid, "Biweekly surveys",    avg_days = nrow(panel) / n_distinct(panel$pid)),
  summ_basic(steam_clean,    pid, "Steam",               avg_days = avg_days_platform("Steam")),
  summ_basic(nintendo_clean, pid, "Nintendo",            avg_days = avg_days_platform("Nintendo")),
  summ_basic(xbox_clean,     pid, "Xbox",                avg_days = avg_days_platform("Xbox"))
) |>
  mutate(
    `Unique participants` = number(`Unique participants`, accuracy = 1, big.mark = ","),
    `Total events`        = number(`Total events`,        accuracy = 1, big.mark = ",", scale_cut = cut_long_scale()),
    `Total hours`         = number(`Total hours`,         accuracy = 1, big.mark = ",", scale_cut = cut_long_scale()),
    `Avg events / participant` = number(`Avg events / participant`, accuracy = 0.1, big.mark = ",", scale_cut = cut_long_scale()),
    `Avg active days`          = number(`Avg active days`,          accuracy = 0.1, big.mark = ",", scale_cut = cut_long_scale())
  )
...
plot_data <- list(
  ...
  events_per_participant(steam_clean,    pid, cap = Inf),
  events_per_participant(nintendo_clean, pid, cap = Inf),
  events_per_participant(xbox_clean,     pid, cap = Inf)
)
...
spark_density <- function(d, xmax_t, ...) {
  d <- data.frame(x = xf(d))
  ggplot(d, aes(x = x)) +
    geom_density(linewidth = LINEWIDTH, fill = "grey60", alpha = 0.35, adjust = 1) +
    scale_x_continuous(
      limits = c(0, xmax_t),
      expand = c(0, 0),
      breaks = c(0, xmax_t),
...
tt(
  ...
  notes = paste0(
    ...
    "Winsorized at ", round(CAP_PROB*100), "th percentile",
...
# --- counts ---
n_qualified <- intake |> filter(qualified) |> summarise(n = n_distinct(pid)) |> pull()
...
sr_long <- intake |>
  ...
  pivot_longer(c(Steam, Xbox, PlayStation, Nintendo, iOS, Android),
               names_to = "platform", values_to = "minutes") |>
  group_by(platform) |>
...
telemetry_weekly_A <- daily_all |>
  ...
  group_by(platform) |>
  summarise(minutes = mean(total_minutes, na.rm = TRUE) / n_qualified, .groups = "drop") |>
...
ghost <- weekly_by_platform |>
  ...
  mutate(
    ...
    xmin = xmid - 0.45, xmax = xmid + 0.45,
    ymid = (ymin + ymax) / 2
  ) |>
  filter(gap > 0)

# --- Panel A plot (your original plot code works as-is) ---
gA <- ggplot(weekly_by_platform, aes(x = source, y = minutes, fill = platform)) +
  geom_col(width = 0.9) +
  scale_fill_viridis_d(option = "magma", begin = .1, end = .8, limits = stack_order) +
  labs(x = "Data Source", y = "Weekly minutes per qualified person", fill = "Platform") +
  theme(legend.position = "bottom") +
  geom_text(aes(label = ifelse(minutes < 10, "", round(minutes, 0))),
            position = position_stack(vjust = 0.5), color = "white", family = "serif") +
  geom_rect(data = ghost, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
            inherit.aes = FALSE, linetype = "dotted", linewidth = 0.6, color = "black", fill = NA) +
  geom_text(data = ghost, aes(x = xmid, y = ymid, label = paste0("+", round(gap, 0))),
...
sr_users <- intake |>
  ...
  pivot_longer(c(Steam, Nintendo, Xbox), names_to = "platform", values_to = "sr_minutes") |>
  mutate(uses = case_when(
...
gBCD <- ggplot(bcd, aes(source, minutes, fill = source)) +
  geom_col(width = 0.8) +
  scale_fill_viridis_d(option = "magma", begin = .15, end = .7, guide = "none") +
...
slot_play <- hourly_all |>
  ...
  summarise(minutes = pmin(sum(minutes, na.rm = TRUE), 60 * n()), .groups = "drop")
...
slot_summary <- slot_per_player |>
  ...
  mutate(dow = factor(dow, levels = 1:7,
