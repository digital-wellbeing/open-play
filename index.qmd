...
nHours <- number(sum(daily_all$minutes/60), accuracy = 0.1, big.mark = ",", scale_cut = cut_long_scale())
nUsers <- number(n_distinct(daily_all$pid), accuracy = 1, big.mark = ",")
nGames <- number(n_distinct(hourly_all$title_id), accuracy = 1, big.mark = ",")
propFailedDailyAttnCheck <- number(sum(daily$bpnsfs_failed_att_check)/nrow(daily)*100, accuracy = .1)
propFailedBiweeklyAttnCheck <- number(sum(biweekly$bangs_failed_att_check)/nrow(biweekly)*100, accuracy = .1)

propDroppedXboxSessions <- number((1-(nrow(xbox) / nrow(xbox_raw))) * 100, accuracy = 0.1)
...
# --- local timezone for this participant (fallback to "UTC" if missing) ---
tz_local <- intake |>
  filter(pid == person_id) |>
  summarise(tz = first(na.omit(local_timezone))) |>
  pull(tz)
if (length(tz_local) == 0) tz_local <- "UTC"

# --- 4am→4am window in LOCAL time ---
window_start <- as.POSIXct(paste(survey_day - 1, "04:00:00"), tz = tz_local)
window_end   <- as.POSIXct(paste(survey_day,     "04:00:00"), tz = tz_local)

# --- helper: overlap + clip to window (expects start/end cols) ---
...
# --- SESSIONS (UTC -> LOCAL using participant tz) ---
sessions_df <- sessions_all |>
  filter(pid == person_id) |>
  transmute(
    platform = if ("platform" %in% names(.)) platform else "Gaming",
    start = ymd_hms(session_start, tz = "UTC") |> with_tz(tz_local),
    end   = ymd_hms(session_end,   tz = "UTC") |> with_tz(tz_local)
  ) |>
  clip_to_window()

# --- SURVEY (previous day's submission that falls inside the window; already local) ---
...
activity_colors <- c(
  "Sleep"                      = "#355C7D",
  "Personal Care"              = "#6C5B7B",
  "Video Gaming"               = "#2A9D8F",
  "Offline Leisure & Social Life" = "#E76F51",
...
ggplot() +
  ...
  # Survey lane (bottom)
  geom_point(
    data = survey_df, aes(x = ts, y = 1),
    shape = 21, size = 3, stroke = 0.6, fill = "#FF6B35"
  ) +
  geom_segment(
    data = survey_df, aes(x = ts, xend = ts, y = 0.6, yend = 3.4),
  ...
  scale_y_continuous(
    breaks = c(1, 2, 3), labels = c("Survey", "Gaming", "Diary"),
  ...
  labs(
    x = paste0(
      "Local time from ", format(window_start, "%Y-%m-%d %H:%M"),
      " to ", format(window_end, "%Y-%m-%d %H:%M"), " (", tz_local, ")"
    ),
    y = NULL,
    title = paste("Example 4am–4am day for", person_id)
  ) +
...
1.  In each daily and biweekly survey, one item from the BANGS (daily) and BPNSFS (biweekly) was duplicated to assess response consistency [@MeadeCraig2012identifying]; participants whose responses to the two identical items differed by more than one scale point were flagged for potential careless responding (daily surveys: `r propFailedDailyAttnCheck`% flagged, biweekly: `r propFailedBiweeklyAttnCheck`%).
2.  In the telemetry, we use several heuristics to identify potential unreliable sessions: (a) sessions beginning or ending in the future (indicative of clock manipulation or other errors), (b) sessions longer than 10 hours (indicative of substantial idle time or background usage), (c) 3 or more games played simultaneously (indicative of inactivity on multiple games or data corruption). In total, we excluded TODO
3.  In the time use, we flag any cases with fewer than 5 distinct activities logged in a day, or that are missing any TODO
