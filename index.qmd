...
biweekly_raw <- read_csv("data/raw/survey_biweekly_raw.csv.gz")
```
...
---
abstract: |
  A major limitation to understanding digital technology use, and its potential psychological consequences, is the lack of sufficiently detailed, multidimensional, and accurate data. We present a dataset of `{r} n_participants` individuals' video game play telemetry data from Nintendo Switch, Steam, and Xbox, paired with psychological assessments across multiple dimensions of mental health, motivations, well-being, and cognitive ability. The data were collected under a preregistered design that included 12 weeks of survey data (thirty daily assessments, six biweekly surveys, three biweekly cognitive tests), and digital trace data for `{r} n_months` months. Cleaned data include `{r} n_hours` hours of video game play across `{r} n_games` titles, `{r} n_surveys` responses to 14 survey instruments, and `{r} n_simon` attention ability measures to facilitate examining longitudinal associations between play behaviors and psychological functioning. Data and codebook are available under a {{< meta data-license >}} license at {{< meta data-url >}}.
---
...
Here, we present a longitudinal dataset of digital trace data across multiple gaming platforms consisting of `r n_participants` participants, `r n_daily` daily and `r n_biweekly` biweekly surveys, `r n_simon` behavioral attention assessments, and a total of `r n_hours` of gameplay hours distributed across `r n_sessions` sessions. Digital trace data was sourced from five platforms---Xbox, Nintendo, Steam, iOS, and Android---through distinct pipelines detailed below. The data collection methods were preregistered as part of a Stage 1 registered report (<https://osf.io/pb5nu>).

The dataset is openly available under a {{< meta data-license >}} license at <https://doi.org/10.5281/zenodo.17536656> for unrestricted reuse. @tbl-overview shows a high-level overview of the dataset.
...
rows <- list(
  ...
  list_rbind() |>
  arrange(
    factor(data_type, levels = c("Survey", "Telemetry", "Task")),
    factor(
      measure,
      levels = c(
        "Daily Diary",
