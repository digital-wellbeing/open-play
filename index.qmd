...
biweekly_raw <- read_csv("data/raw/survey_biweekly_raw.csv.gz", guess_max = 5000)
```
...
participant_age <- (function(x) sprintf("%s years old (10th percentile: %s, 90th percentile: %s)", 
                                        print_num(median(x), .1), 
                                        print_num(quantile(x, 0.10), .1), 
                                        print_num(quantile(x, 0.90), .1)))(na.omit(intake$age[intake$qualified]))
...
n_games <- number(n_distinct(hourly_telemetry$title_id), accuracy = 1, big.mark = ",")
...
sessions_df <- hourly_telemetry |>
  ...
  transmute(
    ...
    end   = as.POSIXct(hour_start_local) + lubridate::dminutes(pmax(pmin(minutes, 60), 0))
  ) |>
  clip_to_window()
...
ggplot() +
  ...
  guides(
    fill  = guide_legend(title = "Diary activity",
                         override.aes = list(alpha = 1),
                         keywidth = unit(6, "mm"),
                         keyheight = unit(6, "mm")),
    color = guide_legend(title = "Platform",
