...
n_hours <- print_num(
  (sum(nintendo$duration, na.rm = TRUE) +
    ...
    sum(steam_sum$minutes, na.rm = TRUE)) /
    60
)
...
|              |              | c            | Switch       |
|              |              | c            | playtime     |
|              |              | ounts.ninten | [@BallouEtAl2025Perceived]  |
|              |              | do           |             |
|              |              | .            |             |
|              |              | com/qrcode\> |  |
+--------------+--------------+--------------+--------------+
...
Eligible participants were invited to complete 6 waves of panel surveys,
...
the first panel surveys. Running survey types concurrently allows us to
...
participant and remained available until 3am. Panel survey links were
...
steam_weekly <- steam_sum |>
  ...
  summarise(
    ...
    minutes_per_player = sum(minutes, na.rm = TRUE) /
      n_distinct(intake$pid[intake$qualified])
  ) |>
  summarise(
...
xbox_weekly <- xbox_clean |>
  ...
  summarise(
    ...
    minutes_per_player = sum(duration, na.rm = TRUE) /
      n_distinct(intake$pid[intake$qualified])
  ) |>
  summarise(
    minutes = mean(minutes_per_player, na.rm = TRUE)
...
ghost <- weekly_by_platform %>%
  ...
  mutate(
    ...
    reason = dplyr::case_match(
      ...
      "Steam" ~
        "Uncaptured Steam play\n(e.g., invisible mode, account sharing)",
...
ggplot(weekly_by_platform, aes(x = source, y = minutes, fill = platform)) +
  ...
  coord_cartesian(
    clip = "off",
    ylim = c(0, max(ghost$ymax) * 1.08),
    xlim = c(1, max(ghost$xmax) * 1.08)
  )
```
